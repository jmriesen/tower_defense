...
use crate::movement::{path::PathFollowing, Movement};
...
impl EnemyFactory {
    fn update(&mut self, data: &mut EnemyDataStorage) -> Option<Entity> {
        ...
        if number != 0
            && (self.stopwatch == Stopwatch::Waiting || self.stopwatch.elapsed() > spacing)
        {
            number -= 1;
            self.command = if number != 0 {
                Some(SpawnEvent {
                    number,
                    spacing,
                    health,
...
impl<'s> System<'s> for SpawnSystem {
    ...
    fn run(&mut self, (channel, mut factories, mut transforms, mut data): Self::SystemData) {
        ...
        let configs: Vec<(Option<Entity>, Transform)> = (&mut factories, &mut transforms)
            .join()
            .map(|(factory, transform)| (factory.update(&mut data), transform.clone()))
            .collect();
