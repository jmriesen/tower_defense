...
impl From<Transform> for LatticePoint {
    fn from(transforms: Transform) -> Self {
        LatticePoint::from(TilePoint::from(transforms))
    }
}
impl From<LatticePoint> for Transform {
    fn from(point: LatticePoint) -> Self {
        Transform::from(TilePoint::from(point))
    }
}
...
impl From<TilePoint> for Transform {
    fn from(point: TilePoint) -> Self {
        ...
        transform.set_translation_xyz(point.x * TILE_SIZE as f32, point.y * TILE_SIZE as f32, 0.);
        transform
    }
}
